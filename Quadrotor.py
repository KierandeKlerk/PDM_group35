...
class Quadrotor(core.Env):
    ...
    def step(self, cmd_rotor_speeds):
        ...
        action = [w1, w2, w3, w4]
        '''
        rotor_speeds = cmd_rotor_speeds ##Speed check already done in the controller        
        rotor_thrusts = np.zeros(4)
        for i in range(len(rotor_thrusts)):
            rotor_thrusts[i] = self.k_thrust * rotor_speeds[i]**2
        '''
        Next, [w1, w2, w3, w4] into [F Mx My Mz]
        '''
        
        #Calculating force F (or T) and moments Mx My and Mz (in M)
        T = np.matmul(self.to_TM[0, :], rotor_thrusts)
        M = np.matmul(self.to_TM[1:4, :], rotor_thrusts)
    ...
    def _s_dot_fn(self, t, s, u1, u2):
        ...
        v_dot = 1/self.mass * ((np.matmul(r.as_matrix(), np.array([0, 0, u1]))) + self.weight)
